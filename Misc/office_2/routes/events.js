...
router.get("/:id", (req, res) => {
    ...
    var names = [];
    events.getEvent(parseInt(req.params.id)).then((eventObj) => {

        var peoplePromises = eventObj.attendees.map(attendee => people.getPerson(attendee));

        Promise.all(peoplePromises).then((arrayOfPeople) => {
        ...
        return Promise.all([eventObj, locations.getLocation(eventObj.location)]).then((values) => {
            ...
            res.render("events/singleEvent", { singleEvent: singleEventObj, name: names });
        });
...
router.get("/", (req, res) => {
    ...
    events.getAllEvents().then((eventsList) => {
